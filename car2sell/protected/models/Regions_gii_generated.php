...
class Regions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, geo_id, reg_domain', 'required'),
			array('name', 'length', 'max'=>70),
			array('geo_id', 'length', 'max'=>2),
			array('reg_domain', 'length', 'max'=>30),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, geo_id, reg_domain', 'safe', 'on'=>'search'),
		);
	}
